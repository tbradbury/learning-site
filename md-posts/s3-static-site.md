...
In the last two blogs we looked at [ECS](/posts/ecs-basics) and then [EKS](/posts/eks-basics) both provide great ways for us to deploy/host our site. However both solutions are a bit over the top for our simple blog site (certainly cost more to run on AWS).
...
We also need to change the next.config.js file to ensure our static html plays nice with S3. As it stands our posts will be build as /posts/name-of-post.html. When you have your static site up and runing and you go from the homepage to this url sitename.com/posts/name-of-post it will be fine and you will see your page, but if you refesh you will get a 404 page not found error / or direct you back to the homepage depending on your setup. This is due to how AWS handles request to objects in your S3 bucket. 
...
AWS Simple Storage Service or S3 is a object storage service. You can store data on the cloud, but for our purposes you can also host a static website there (at an incredibly low price).
...
This job builds are static html into the out/ file then deploys it to our S3 bucket. The job has a parameter called argument, with a default value of `--delete` this will ensure when we do a new deploy we replace the old files with the new one. Alternatively we could keep the old files and have versions (if we need to go back to an old version, but for now this is fine).

We will add the job to our 2 workflows for our PR branchs we wont deploy but we will confirm the deploy will work by changing our argument param:
...
This gives read access to everyone i.e it will be visiable for internet traffic.

**5) Cloudfront**

You could leave it there you have a static site, but there are a couple of issues, the domain name is not really great for users `http://example.co.uk.s3-website-eu-west-1.amazonaws.com/` plus its http not https.

We can sort the https with **Cloudfront** which gives use the added benifits of a CDN (edge servers closer to users).

Go to **Cloudfront** in AWS console and set up a new distribution (web distribution). 

In the **Origin Domain Name** section you will get a dropdown with your S3 bucket name, but you don't want to use this you need to add instead the S3 bucket static site url `http://example.co.uk.s3-website-eu-west-1.amazonaws.com/`. The rest of the setting for now you can leave as is.
...
This is better but still not great, it would be better if we could add our own domain `example.co.uk`.

**5) Certificate Manager with third party domain provider**

We could provide a domain using **Route 52** but I already have a domain with a third party provider.

Go to **Certificate Manager** in the AWS console and click on **Provision certificates** (note to work with **Cloudfront** this needs to be in us-east-1 region regardless of where your s3 bucket or cloudfront were set up)

Add your domain choose DNS validation, this will give you a key/value pair that you need to add in your DNS provider console as a CNAME record. This enables AWS to verifiy you have the right to use this domain. Once the domain in **Certificate Manager** has status **issues** you can use your domain in **Cloudfront**.
